...
def speak(my_text, language="en"):
    tts = gTTS(my_text, lang=language)
    filename = "Yolov4-Detector-and-Distance-Estimator-master/aud.mp3"
    tts.save(filename)
...
while True:
    ...
    for d in data:
        ...
        speak(str(d[0]) + " " + "is present at distance of " + str(round(distance, 2)))
        if (round(distance, 2) < 20):
            speak(str(d[0]) + " is coming closer to you")
